...
# escl/ lives one level below the reference driver; add it to the path.
...
_STATE = threading.Lock()              # guards _JOB and job state transitions
_JOB   = None                          # the single current job (hardware is exclusive)
FAKE   = False                         # --fake: instant synthetic image, no hardware
_MAX_BODY = 256 * 1024                 # cap on a ScanSettings POST body (bytes)
...
def _run_job(job):
    ...
    try:
        if FAKE:
            ...
            with _STATE:
                job.data = data
                if job.state != "Aborted": job.state = "Completed"
            _log("job %s: FAKE done, %d bytes" % (job.id[:8], len(data)))
            return
        with _LOCK:
            driver.open_device()
            try:
        ...
        with _STATE:
            job.data = data
            if job.state != "Aborted": job.state = "Completed"   # keep a mid-scan cancel
        _log("job %s: done, %d bytes" % (job.id[:8], len(data)))
...
def parse_settings(body):
    ...
    # Reject DTDs/entity definitions to avoid entity-expansion (billion-laughs) DoS.
    low = body.lower()
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_DELETE(self):
        if "/eSCL/ScanJobs/" not in self.path:
            return self._empty(404)
        jid = self.path.split("?", 1)[0].rstrip("/").split("/eSCL/ScanJobs/", 1)[1].split("/", 1)[0]
        with _STATE:
            ok = (_JOB is not None and _JOB.id == jid)          # only cancel the named job
    ...
    def _create_job(self):
        ...
        jid = uuid.uuid4().hex
        busy = False
        with _STATE:                                           # atomic busy-check + assign
    ...
    def _next_document(self, path):
        ...
        while job.state in ("Pending", "Processing") and time.time() - t0 < 300:
            time.sleep(0.25)
        # atomically claim the single page so a duplicate/racing GET can't resend it
        with _STATE:
            if job.state == "Aborted":
                decision, data = "abort", None
            elif job.delivered or job.data is None:
                decision, data = "empty", None
            else:
                job.delivered = True
                decision, data = "send", job.data
        if decision == "abort":
            return self._empty(500)
        if decision == "empty":
            _log("NextDocument: nothing more to send -> 404")
            return self._empty(404)          # flatbed: one page, then done
        _log("NextDocument: sending %d bytes (%s)" % (len(data), job.ctype))

...
def _run_scan(a):
    ...
    if not [f for f in (a.format or '').split(',') if f.strip()]:
        print('bad --format: give at least one of png,tif,jpg,pdf,raw')
        sys.exit(2)
    ...
    driver.close_device()
    if not written:
        print('ERROR: no files written (unknown --format? valid: png,tif,jpg,pdf,raw)')
        sys.exit(2)
